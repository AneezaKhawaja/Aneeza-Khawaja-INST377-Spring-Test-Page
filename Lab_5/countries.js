...
const generateRandom=(CountriesTotal,memory)=>{
    let rand=Math.floor(Math.random()*CountriesTotal+1);
    for(let a=0;a<memory.length;a++){
        if(memory[a]==rand){
            return generateRandom(CountriesTotal,memory);
        }
    }
    memory.push(rand);
    return rand;
...
async function start(){
    ...
    const CountriesTotal=countryCodes.length;
    const MAX=25;
    let randomArr=[];
    let memory=[];
    for(let a=0;a<MAX;a++){
        rand=generateRandom(CountriesTotal,memory);
        let temp={
